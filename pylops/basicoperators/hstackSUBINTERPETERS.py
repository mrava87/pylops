...
import sys
import concurrent.futures as mti
...
from pylops.utils.typing import NDArray, Tinoutengine
...
class HStack(LinearOperator):
    ...
    nproc : :obj:`int`, optional
        Number of processes/threads/subinterpreters used to evaluate the N 
        operators in parallel. If ``nproc=1``, work in serial mode.
    ...
    parallel_kind : :obj:`str`, optional
        ...
        ``multiprocessing``), ``multithread`` (using
        ``concurrent.futures.ThreadPoolExecutor``),  ``subinterp`` (using
        ``concurrent.futures.InterpreterPoolExecutor``), or ``auto`` (
        ``multiproc`` if GIL is enabled, ``multithread`` otherwise).
    ...
    pool : :obj:`multiprocessing.Pool` or :obj:`concurrent.futures.ThreadPoolExecutor` or :obj:`concurrent.futures.InterpreterPoolExecutor` or :obj:`None`
    ...
    def __init__(
        ...
        parallel_kind: str = "auto",
        dtype: Optional[str] = None,
    ) -> None:
        # Check parallel kind and assign value if auto
        if parallel_kind not in ["multiproc", "multithread", "subinterp", "auto"]:
            raise ValueError(
                "parallel_kind must be 'multiproc', 'multithread', "
                "'subinterp', or 'auto'"
            )
        if parallel_kind == "auto":
            parallel_kind = "multiproc" if sys._is_gil_enabled() else "multithread"
        self.ops = ops
        ...
        if len(set(dimsd)) == 1:
            ...
            dimsd = (self.nops,)
            forceflat = True
        # create pool for multiprocessing / multithreading / subinterpreters
        ...
        if self.nproc > 1:
            ...
            elif parallel_kind == "subinterp":
                self.pool = mti.InterpreterPoolExecutor(max_workers=nproc)
            else:
                self.pool = mti.ThreadPoolExecutor(max_workers=nproc)
    ...
    def nproc(self, nprocnew: int):
        ...
        if nprocnew > 1:
            ...
            elif self.parallel_kind == "subinterp":
                self.pool = mti.InterpreterPoolExecutor(max_workers=nprocnew)
            else:
                self.pool = mti.ThreadPoolExecutor(max_workers=nprocnew)
                self.lock = threading.Lock()
    ...
    def _matvec_subinterp(self, x: NDArray) -> NDArray:
        ys = list(
            self.pool.map(
    ...
    def _rmatvec_multithread_subinterp(self, x: NDArray) -> NDArray:
        ys = list(
            self.pool.map(
                lambda args: _matvec_rmatvec_map(*args),
                [(oper._rmatvec, x) for iop, oper in enumerate(self.ops)],
    ...
    def _rmatvec(self, x: NDArray) -> NDArray:
        if self.nproc == 1:
            ...
            if self.parallel_kind == "multiproc":
                ...
                y = self._rmatvec_multithread_subinterp(x)
        return y
